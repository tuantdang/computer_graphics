...
def draw_scene(triangles, vertices):
    vertices = scale(vertices, [0.5, 0.5, 0.5])

    for i in range(10):
        draw_box(triangles, translate(vertices, [1*i, 0, 0]), colors[rand()%6])
    
    # First column
    for i in range(10):
        draw_box(triangles, translate(vertices, [0, 1*i, 0]), colors[rand()%6])

    # Second column (last)
    vertices1 = translate(vertices, [1*10, 0, 0])
    for i in range(10):
        draw_box(triangles, translate(vertices1, [0, 1*i, 0]), colors[rand()%6])
